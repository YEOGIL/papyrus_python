...
def generate_img():
    try:
        # 클라이언트에서 전달된 프롬프트(archived 텍스트)를 받아옴
        data = request.json
        prompt = data.get("prompt", None)
        # prompt = "A cat"
        if not prompt:
            return jsonify({"isSuccessful": False, "result": "프롬프트가 제공되지 않았습니다."}), 400

        # OpenAI 이미지 생성 API 호출: 1024x1024 해상도, 3장 생성
        images_urls = []
        for _ in range(1):  # 원하는 장 수만큼 반복
            response = client.images.generate(
                model="dall-e-2",
                prompt=(
                    "아래의 텍스트와 어울리는 분위기의 이미지를 생성 해주세요.\n"
                    f"- 내용: {prompt}"
                ),
                # prompt=prompt,
                size="512x512",
        ...
        return jsonify({"isSuccessful": True, "images": images_urls})
    
    except Exception as e:
        app.logger.exception("Error in generate_img")
        return jsonify({"error": str(e)}), 500
